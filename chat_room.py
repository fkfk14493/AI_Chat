        ...
        # ── [일반 대화] 특수 명령어가 아닌 일반 티키타카일 때 ──
            ...
            with st.chat_message("assistant", avatar=tgt_avatar):
                with st.spinner("답장 하는 중..."):
                    ...
                    # 성공한 응답의 이번 요청 토큰만 전역 세션에 누적하고 DB에 저장!
                    if response.usage_metadata:
                        input_tokens = response.usage_metadata.prompt_token_count or 0
                        output_tokens = response.usage_metadata.candidates_token_count or 0
            ...
            # ==========================================
            if len(st.session_state.messages) >= 40 and len(st.session_state.messages) % 20 == 0:
                with st.spinner("예전 기억들을 요약하는 중..."):
                    # 최근 10턴은 직접 기억
                    keep_messages = st.session_state.messages[-20:]
                    # 그 직전 10턴만 새롭게 요약에 추가
                    old_messages = st.session_state.messages[-40:-20]
                    ...
                    summary_prompt = f"""
                    너는 장편 소설의 연속성을 관리하는 기록 담당자다.
                    
                    [기존 줄거리]와 [새로 추가된 대화 기록]을 합쳐,
                    다음 대화에 필요한 핵심 정보를 최대 2000자 이내로 누적 요약하라.
                    정보가 적다면 억지로 길게 쓰지 마라.
                    ...
                    - 새로 밝혀진 설정이나 비밀
                    - 약속, 목표, 갈등, 미해결 문제
                    - 장소, 소유물, 부상 등 이후 전개에 영향을 주는 상태
                    - 이전 대화에서 확정된 사실
                    
                    규칙:
                    - 기존 줄거리의 중요한 정보를 임의로 삭제하거나 바꾸지 마라.
                    - 새 대화와 기존 줄거리가 충돌하면 가장 최근에 확정된 내용을 우선하라.
                    - 추측하거나 새로운 설정을 만들어내지 마라.
                    - 대사나 장면 묘사는 최소화하고 사실과 감정 변화 중심으로 기록하라.
                    - 이미 기록된 내용을 불필요하게 반복하지 마라.
                    - 다음 대화를 이어가는 데 필요한 정보 위주로 최대 1000자 이내로 작성하라.
                    - 요약문만 출력하고 설명이나 머리말은 쓰지 마라.
